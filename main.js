document.addEventListener('DOMContentLoaded', () => {
    ...
    // 画像パス生成：
    // 1. CSVのiconが埋まっていればそれを使う
    // 2. なければ type + typeIndex から自動生成（weapon_1.png, armor_2.png など）
    // 3. それも読めなければSVGフォールバック
    function getImgSrc(card) {
        if (card.icon && card.icon.trim() !== '') return 'images/' + card.icon;
        if (card.typeIndex) return 'images/' + card.type + '_' + card.typeIndex + '.png';
        return makeSVGUrl(card);
    }

    // imgタグにフォールバック付きでセットするヘルパー
    function setImgWithFallback(imgEl, card) {
        const primary   = getImgSrc(card);
        const secondary = (card.typeIndex)
            ? 'images/' + card.type + '_' + card.typeIndex + '.png'
            : null;
        imgEl.onerror = () => {
            if (secondary && imgEl.src !== window.location.origin + '/' + secondary) {
                imgEl.src = secondary;
            } else {
                imgEl.onerror = null;
    ...
    function showAtkBar(baseCard, totalPower) {
        currentAtk = { baseCard, totalPower };
        atkBarName.textContent  = baseCard.name;
        atkBarPower.textContent = '合計攻' + totalPower;
        atkBar.style.display = 'flex';
    }
    function updateAtkBar(totalPower) {
        if (currentAtk) currentAtk.totalPower = totalPower;
        atkBarPower.textContent = '合計攻' + totalPower;
    }
    function hideAtkBar() {
